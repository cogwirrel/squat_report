...
The user begins by starting up the application. They will immediately see a video feed of the front-facing camera of their Android mobile device. They will see a message at the top of the screen, instructing them in the first step they must take: `Ensure surroundings are still.'. The user can achieve this by placing their device in a stationary location and waiting for the application's view to clear.
...
As the model is displayed and the application beeps, the application's voice will indicate the next move expected from the user by saying `Start squatting when ready'. The user may now begin squatting in their own time. As they squat, the position of the skeleton on screen will be updated in real-time, as shown in figure~\ref{fig:midsquat}.
